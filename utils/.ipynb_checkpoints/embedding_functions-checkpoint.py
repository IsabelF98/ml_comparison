...
def Laplacain_Eigenmap(data_df,k,n,metric):
    ...
    INPUTS
    ------
    data_df: data which you are dimensionally reducing
    k: k value for k-Nearest Neighboor affinity matrix
    n: number of dimensions you are reducing your data to
    metric: distance metric you are using to construct affinity matrix (correlation, cosine, euclidean)
    
    OUTPUTS
    -------
    LE_data: dimensionaly reduced data (both original and normalized)
    ...
    X_affinity       = kneighbors_graph(data_df, n_neighbors=k, include_self=True, metric=correlation, n_jobs=52) # Affinity matrix
